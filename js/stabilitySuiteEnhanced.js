...
// ============================================
// Enhanced Metric Calculation with Coupling
// ============================================
...
function applyCoupling(metrics) {
  // Coupling weights: SD, ENT, AF, AD influence each other
  const weights = [
  ...
  for (let i = 0; i < 4; i++) {
    ...
    coupled[i] = Math.tanh(coupled[i]); // Non-linear activation
  }
  return coupled;
}
...
function computeDynamicEPS(turns) {
  ...
  // History for coupling
  let prevDrift = 0;
  let prevAssumption = 0;
  let prevAffective = 0;
  let prevEntropy = 0.5;
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Raw entropy (initial high, decreases with turns)
    const entropy = Math.max(0.1, 0.8 - i * 0.08);
    rawEntropy.push(entropy);
    
    // Raw assumption (based on text features)
    let assumption = 0.1;
    ...
    // Raw affective (based on intensity markers)
    let affective = 0.2;
  ...
  const enhancedAffective = [rawAffective[0]];
  const couplingHistory = [];
  
  for (let i = 1; i < n; i++) {
    // Enhanced drift influenced by entropy
    const drift = enhancedDrift(rawDrifts[i], enhancedDrifts[i-1], enhancedEntropy[i-1]);
    enhancedDrifts.push(drift);
    
    // Enhanced entropy influenced by drift (non-zero now!)
    ...
    // Enhanced assumption with decay and drift influence
    const assumption = enhancedAssumption(rawAssumptions[i], enhancedAssumptions[i-1], drift);
    enhancedAssumptions.push(assumption);
    
    // Enhanced affective with memory and entropy influence
    const affective = enhancedAffective(rawAffective[i], 0.8, enhancedAffective[i-1], enhancedEntropy[i-1]);
    enhancedAffective.push(affective);
  ...
  // Final coupled metrics (last step)
  ...
  // Advanced stability index with propagation
  ...
  const lastAffective = enhancedAffective[enhancedAffective.length - 1];
  ...
  return {
    curves: {
      ...
      affectiveDimension: enhancedAffective
    },
    metrics: {
...
// ============================================
// Perturbation Tests with Enhanced Metrics
// ============================================
...
function runEnhancedStabilityTests() {
  ...
  for (const test of tests) {
    ...
    // Enhanced Stability Index (includes propagation intensity)
